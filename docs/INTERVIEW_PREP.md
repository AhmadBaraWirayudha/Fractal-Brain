...
tested with 162 checks across a smoke suite and pytest, all currently green,
...
| pytest suite | 25 tests, all passing (started at 14, across two review passes) |
| Total automated checks | 162 |
...
| Bugs found+fixed in the latest full review | 5 correctness bugs, 10 completeness/testing/docs gaps, plus 2 more (a hardcoded Unix-only path, a missing auto-install for a test dependency) found by actually running it on Windows |
...
Use the 137/25/162 numbers specifically and precisely if asked "how do you
...
25-test pytest suite (engine behavior, CLI, regressions) that now also runs
